...
export const GoalsView: React.FC<GoalsViewProps> = ({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const target = parseFlexibleCurrency(form.targetAmount);
    const current = form.currentAmount
      ? parseFlexibleCurrency(form.currentAmount)
      : 0;
  ...
  const handleDeposit = async (goal: Goal) => {
    ...
    const amount = parseFlexibleCurrency(amountStr);
  ...
  const openEditModal = (goal: Goal) => {
    ...
    setForm({
      title: goal.title,
      targetAmount: goal.targetAmount.toLocaleString(locale, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }),
      currentAmount: goal.currentAmount.toLocaleString(locale, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }),
  ...
  const confirmDeposit = async (e: React.FormEvent) => {
    ...
    const amount = parseFlexibleCurrency(depositAmount);
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6 px-2">
        <div>
          <p className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-[0.3em] mb-1">
            {t("common.management")}
          </p>
      ...
      {isLoading || isInitialLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-72 bg-slate-100 dark:bg-slate-900 rounded-[2.5rem] animate-pulse"
            ></div>
          ))}
        </div>
      ) : loadError ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {goals.map((goal) => {
            ...
            return (
              <div
                key={goal.id}
                className="glass-card p-4 sm:p-8 md:p-10 rounded-2xl sm:rounded-[2.5rem] md:rounded-[3rem] relative overflow-hidden group hover:translate-y-[-6px] transition-all duration-300"
              >
                {isComplete && (
                  ...
                  <div className="flex gap-2">
                    <div className="flex gap-2 bg-slate-50 dark:bg-slate-900/50 p-1.5 rounded-2xl border border-slate-100 dark:border-slate-800">
                      <button
                        ...
                        disabled={isReadOnly}
                        className={`p-2.5 text-slate-400 hover:text-cyan-500 dark:hover:text-cyan-400 hover:bg-white dark:hover:bg-slate-800 rounded-xl transition-all shadow-sm ${isReadOnly ? "opacity-50 cursor-not-allowed" : ""}`}
                        title={t("common.edit")}
                      >
                      ...
                      <button
                        ...
                        disabled={isReadOnly}
                        className={`p-2.5 text-slate-400 hover:text-rose-500 dark:hover:text-rose-400 hover:bg-white dark:hover:bg-slate-800 rounded-xl transition-all shadow-sm ${isReadOnly ? "opacity-50 cursor-not-allowed" : ""}`}
                    ...
                    <button
                      ...
                      disabled={isReadOnly}
                      className={`p-4 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-2xl transition-all active:scale-90 shadow-sm border border-emerald-100 dark:border-emerald-500/20 ${isReadOnly ? "opacity-50 cursor-not-allowed" : ""}`}
                ...
                <div className="space-y-1 mb-8 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="text-2xl font-black text-slate-800 dark:text-white tracking-tight group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors truncate">
                      {goal.title}
                    </h3>
                    <ReadOnlyBadge type="goal" id={goal.id} />
                  </div>
                  <p
                    className={`text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-[0.2em] ${isPrivacyEnabled ? "blur-sm select-none" : ""}`}
                  >
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-end">
                    <div className="space-y-1">
                      ...
                      <span
                        className={`text-3xl font-black text-slate-800 dark:text-white tracking-tighter ${isPrivacyEnabled ? "blur-md select-none" : ""}`}
                      >
                        {isPrivacyEnabled
                          ? "•••"
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span
                        className={`text-[10px] font-black px-3 py-1.5 rounded-xl uppercase tracking-widest ${isComplete ? "bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400" : "bg-cyan-50 dark:bg-cyan-500/10 text-cyan-600 dark:text-cyan-400"}`}
                      >
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[3rem] w-full max-w-md shadow-2xl p-10 animate-in zoom-in-95 duration-300 border border-slate-200 dark:border-slate-800">
            <div className="flex justify-between items-center mb-10">
              ...
              <button
                ...
                className="p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all active:scale-95 shadow-sm"
              >
            ...
            <form onSubmit={handleSave} className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="relative group">
                    <span className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 font-black text-xs pointer-events-none group-focus-within:text-cyan-500 transition-colors">
                      {currencySymbol}
                    </span>
                    <input
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          currentAmount: formatInputCurrency(e.target.value),
                        })
                      }
                    />
                  </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[3rem] w-full max-w-md shadow-2xl p-10 animate-in zoom-in-95 duration-300 border border-slate-200 dark:border-slate-800">
            <div className="flex justify-between items-center mb-10">
              <div>
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-widest mt-1">
                  {t("goals.goalTitle")}: {selectedGoal.title}
                </p>
              </div>
              <button
            ...
            <form onSubmit={confirmDeposit} className="space-y-8">
              <div>
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-4 ml-1">
                  {t("goals.depositAmount")}
                </label>
                <div className="relative group">
                  <span className="absolute left-8 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-600 font-black text-2xl pointer-events-none group-focus-within:text-cyan-500 transition-colors">
                    {currencySymbol}
                  </span>
                  <input
                    autoFocus
                    ...
                    value={depositAmount}
                    onChange={(e) =>
                      setDepositAmount(formatInputCurrency(e.target.value))
                    }
                  />
                </div>
              </div>

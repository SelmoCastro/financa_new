...
export const InstallmentFormModal: React.FC<InstallmentFormModalProps> = ({
  ...
  const currentTotal = parseFlexibleCurrency(installForm.totalAmount);
  const exceedsLimit =
    creditCardLimit > 0 && creditCardUsed + currentTotal > creditCardLimit;
    ...
    <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-cyan-50/50 dark:bg-slate-950/50 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl bg-white dark:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-white"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="p-6 space-y-4">
          <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-1.5">
            Descrição
          </label>
          <input
            className="w-full p-3.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-bold text-slate-900 dark:text-white outline-none focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500"
            placeholder="Descrição (ex: Notebook, Geladeira)"
            value={installForm.description}
            onChange={(e) =>
              setInstallForm({ ...installForm, description: e.target.value })
            }
          />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setInstallForm({
                    ...installForm,
                    totalAmount: formatCurrencyInput(e.target.value, locale),
                  })
                }
              />
              {creditCardLimit > 0 && (
                <p
                  className={`text-[10px] font-bold mt-1 ${exceedsLimit ? "text-rose-500" : "text-slate-400"}`}
                >
                  {exceedsLimit
                    ? "Excede o limite!"
                    : `Disponível: ${formatCurrency(availableLimit - currentTotal)} / ${formatCurrency(creditCardLimit)}`}
                </p>
              )}
            </div>
            <div>
              ...
              <input
                ...
                value={installForm.installmentCount}
                onChange={(e) =>
                  setInstallForm({
                    ...installForm,
          ...
          <div>
            ...
            <p className="text-[9px] text-slate-500 dark:text-slate-400 mb-2">
              Primeira parcela (deixe 0 se não houver entrada)
            </p>
            <input
              type="text"
              inputMode="numeric"
              ...
              onChange={(e) =>
                setInstallForm({
                  ...installForm,
                  entryAmount: formatCurrencyInput(e.target.value, locale),
                })
              }
            />
          </div>
          ...
          {Number(installForm.installmentCount) > 1 && setUseCustomValues && (
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (newVal) {
                      ...
                      const totalNum = parseFlexibleCurrency(
                ...
                >
                  <span
                    className={`w-4 h-4 rounded border-2 flex items-center justify-center ${useCustomValues ? "bg-cyan-500 border-cyan-500" : "border-slate-300 bg-white"}`}
                  >
                    {useCustomValues && (
                      <svg
                  ...
                  <span
                    className={
                      useCustomValues ? "text-cyan-600" : "text-slate-400"
                    }
                  >
                    Personalizar
                  </span>
                  ...
                  return (
                    <div className="space-y-2">
                        ...
                        <div key={i} className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              newAmounts[i] = formatCurrencyInput(
                                e.target.value,
                                locale,
                              );
                      ...
                      <div className="flex justify-between text-[10px] font-bold pt-1">
                        <span
                          className={
                            diff > 0.02 ? "text-rose-500" : "text-emerald-500"
                          }
                        >
                        ...
                        >
                          {diff > 0.02
                            ? `Falta ${formatCurrency(totalNum - sumValues)}`
                            : "✓ Valores conferem"}
                        </span>
                      </div>
                    </div>
          ...
          <div>
            ...
            <p className="p-3.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-300">
              Dia {installForm.dueDay} (do cartão)
            </p>
          </div>
          ...
          {installmentPreview && (
            <div className="bg-cyan-50 dark:bg-cyan-500/10 rounded-xl p-3 space-y-1.5 border border-cyan-100 dark:border-cyan-500/20">
              <p className="text-[9px] font-black uppercase tracking-widest text-cyan-600 dark:text-cyan-400 mb-1">
                Prévia das parcelas
              </p>
              {installmentPreview.entry > 0 && (
                <div className="flex justify-between text-[11px]">
                  ...
                  <span className="font-black text-amber-700 dark:text-amber-300">
                    {formatCurrency(installmentPreview.entry)}
                  </span>
                </div>
              )}
              {installmentPreview.count >
                (installmentPreview.entry > 0 ? 1 : 0) && (
                <div className="flex justify-between text-[11px]">
                  <span className="text-slate-600 dark:text-slate-300 font-bold">
                    {installmentPreview.entry > 0
                      ? `${installmentPreview.count - 1}x de`
                      : `${installmentPreview.count}x de`}
                  </span>
                  <span className="font-black text-cyan-700 dark:text-cyan-300">
                    {formatCurrency(installmentPreview.perMonth)}
                  </span>
                </div>
              )}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              onClick={onSubmit}
              className={`flex-[2] py-3 text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg ${exceedsLimit ? "bg-slate-300 text-slate-500 cursor-not-allowed shadow-none" : "bg-cyan-600 text-white hover:bg-cyan-700 shadow-cyan-600/20"}`}
            >
              Adicionar
            </button>
          </div>

...
import { encryptAmount } from '../common/services/balance-helper';
...
export class GoalsService {
  ...
  async create(createGoalDto: CreateGoalDto, userId: string) {
    ...
    return this.subscriptionService.createWithLimitCheck(
      ...
      'goal',
      async () => {
        const { deadline, targetAmount, currentAmount, ...rest } =
          createGoalDto;
        return this.prisma.goal.create({
          data: {
            ...
            currentAmount: currentAmount
              ? encryptAmount(currentAmount, this.encryption)
              : encryptAmount(0, this.encryption),
  ...
  update(id: string, updateGoalDto: UpdateGoalDto, userId: string) {
    ...
    if (targetAmount !== undefined)
      data.targetAmount = encryptAmount(targetAmount, this.encryption);

    return this.subscriptionService
      .checkNotExceeding(userId, 'goal', id)
      .then(() =>

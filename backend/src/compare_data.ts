...
const csvContent = fs.readFileSync(
  '../Extrato conta corrente - 022026.csv',
  'latin1',
);
...
for (let i = 1; i < csvLines.length; i++) {
  ...
  if (parts.length >= 5) {
    ...
    if (
      rawDate === '00/00/0000' ||
      description.includes('S A L D O') ||
...
csvUnmatched.forEach((tx) => {
  console.log(
    `${tx.dateStr} | ${tx.type === 'EXPENSE' ? '-' : '+'}${tx.amount.toFixed(2)} | ${tx.description} | ${tx.details}`,
  );
});
...
dbUnmatched.forEach((tx) => {
  ...
  console.log(
    `${dateStr} | ${tx.type === 'EXPENSE' ? '-' : '+'}${tx.amount.toFixed(2)} | ${tx.description} | DB_Cat: ${tx.categoryLegacy}`,
  );
});

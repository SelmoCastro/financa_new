...
async function bootstrap() {
  try {
    ...
    if (missingOptional.length > 0) {
      console.warn(
        `⚠️  Variáveis opcionais ausentes: ${missingOptional.join(', ')}`,
      );
    ...
    if (
      ...
      if (mpToken.includes('TEST-')) {
        ...
        console.error(
          '   Revogue o token de teste e gere um de produção no painel do Mercado Pago.',
        );
        process.exit(1);
      }
      if (!process.env.MERCADOPAGO_WEBHOOK_SECRET) {
        ...
        console.error(
          '   Webhooks de pagamento não podem processar sem assinatura HMAC.',
        );
        process.exit(1);
      }
    }

...
async function parseCSV(filePath: string) {
  ...
  for await (const line of rl) {
    ...
    if (cols.length >= 4) {
      ...
      const type =
        amount >= 0 || typeStr.toLowerCase() === 'entrada'
          ? 'INCOME'
          : 'EXPENSE';
...
async function simularImportacao() {
  ...
  const minDate = new Date(
    Math.min(...csvTransactions.map((t) => t.date.getTime())),
  );
  const maxDate = new Date(
    Math.max(...csvTransactions.map((t) => t.date.getTime())),
  );
  ...
  const existingSet = new Set(
    dbTransactions.map(
      (t) =>
        `${t.date.toISOString().split('T')[0]}_${t.amount}_${t.description.trim().toLowerCase()}`,
    ),
  );
  ...
  for (const t of csvTransactions) {
    ...
    const sameDateAndValue = dbTransactions.filter(
      (dbT) =>
        dbT.date.toISOString().split('T')[0] === dateStr &&
        dbT.amount === t.amount,
    );

    if (existingSet.has(hash)) {
      console.log(
        `[IGNORADA - IDÊNTICA] ${dateStr} - R$ ${t.amount} >> "${t.description}" já existe exatamente igual.`,
      );
      ignoredCount++;
    } else if (sameDateAndValue.length > 0) {
      console.log(
        `[ATENÇÃO - PROVÁVEL DUPLICATA] ${dateStr} - R$ ${t.amount} >> CSV: "${t.description}" | DB: "${sameDateAndValue[0].description}"`,
      );
      ...
      console.log(
        `[NOVA] ${dateStr} - R$ ${t.amount} >> "${t.description}" seria inserida com sucesso.`,
      );
      newCount++;
    }
  }

...
async function migrateCategories() {
  ...
  for (const user of users) {
    ...
    for (const { categoryLegacy } of uniqueCategories) {
      ...
      const isIncome =
        categoryLegacy.toLowerCase().includes('receita') ||
      ...
      console.log(
        `Categoria criada: ${category.name} [${category.type}] para Usuario ${user.id}`,
      );
    ...
    console.log(
      `Conta legado criada e transações vinculadas para usuário ${user.id}`,
    );
  }

...
async function main() {
  ...
  for (const account of accounts) {
    ...
    console.log(`Saldo Registrado: R$ ${Number(registeredBalance).toFixed(2)}`);
    console.log(
      `Saldo Calculado (Transações): R$ ${calculatedBalance.toFixed(2)}`,
    );
    console.log(
      `Diferença: R$ ${(Number(registeredBalance) - calculatedBalance).toFixed(2)}`,
    );
    ...
    if (transactions.length > 0) {
      ...
      transactions
        ...
        .forEach((t) => {
          console.log(
            `- [${t.date.toISOString().slice(0, 10)}] ${t.description}: R$ ${Number(t.amount).toFixed(2)} (${t.type})`,
          );
        });
    } else {
  ...
  if (orphanTransactions.length > 0) {
    ...
    const orphanIncome = orphanTransactions
      .filter((t) => t.type === 'INCOME')
      .reduce((acc, t) => acc + Number(t.amount), 0);
    const orphanExpense = orphanTransactions
      ...
      .reduce((acc, t) => acc + Number(t.amount), 0);
    console.log(
      `Impacto no Geral: R$ ${(orphanIncome - orphanExpense).toFixed(2)}`,
    );
  }
}

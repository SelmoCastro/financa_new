...
export default function ReportsScreen() {
    ...
    if (loading) {
        return (
            <View className="flex-1 bg-slate-50" style={{ paddingTop: insets.top + 20 }}>
                ...
                {[1, 2, 3].map(i => (
                    <View key={i} className="mx-6 mb-4 bg-white rounded-3xl p-6">
    ...
    return (
        <View className="flex-1 bg-slate-50" style={{ position: 'relative' }}>
            <ScrollView
            ...
            >
                ...
                <View style={{ paddingTop: Math.max(insets.top + 20, 50) }} className="px-6 mb-2">
                    <View className="flex-row items-center justify-between">
                        <View>
                            <Text className="text-2xl font-black text-slate-900">Relatórios</Text>
                            <Text className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Análise Financeira</Text>
                        </View>
                        <Pressable
                    ...
                    <View className="px-6 mb-2">
                        <View className="flex-row gap-3">
                            <View className="flex-1 bg-white rounded-2xl p-4 border border-slate-100">
                                <View className="flex-row items-center gap-2 mb-1">
                                    ...
                                    <Text className="text-[10px] font-bold text-slate-400 uppercase">Receitas</Text>
                                </View>
                            ...
                            <View className="flex-1 bg-white rounded-2xl p-4 border border-slate-100">
                                <View className="flex-row items-center gap-2 mb-1">
                                    ...
                                    <Text className="text-[10px] font-bold text-slate-400 uppercase">Despesas</Text>
                                </View>
                            ...
                            <View className="flex-1 bg-white rounded-2xl p-4 border border-slate-100">
                                <View className="flex-row items-center gap-2 mb-1">
                                    ...
                                    <Text className="text-[10px] font-bold text-slate-400 uppercase">Saldo</Text>
                                </View>
                        ...
                        <View className="flex-row gap-3 mt-2">
                            <View className="flex-1 flex-row items-center gap-1">
                                ...
                                <Text className="text-[10px] text-slate-400">vs mês anterior</Text>
                            </View>
                            <View className="flex-1 flex-row items-center gap-1">
                                <MaterialIcons
                ...
                {/* Rule 50/30/20 */}
                {summary?.rule503020 && (
                    <View className="mx-6 mb-4 bg-white rounded-[32px] p-6 border border-slate-100 shadow-sm">
                        <View className="mb-4">
                            <Text className="text-sm font-bold text-slate-800">Regra 50/30/20</Text>
                            <Text className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Distribuição da Renda</Text>
                        </View>
                        ...
                        {ruleSegments.map((seg, i) => (
                            <View key={i} className="flex-row items-center justify-between mb-3">
                                <View className="flex-row items-center gap-2 flex-1">
                                    ...
                                    <Text className="text-xs font-bold text-slate-600">{seg.label}</Text>
                                    <Text className="text-[10px] text-slate-400">ideal {seg.ideal}%</Text>
                                </View>
                                <View className="flex-row items-center gap-2">
                                    <Text className="text-xs font-black text-slate-800" style={isPrivacyEnabled && { color: '#cbd5e1' }}>
                                        {formatValue(seg.value)}
                                    </Text>
                                    <View className={`px-2 py-0.5 rounded-full ${seg.percent <= seg.ideal + 5 ? 'bg-emerald-50' : 'bg-amber-50'}`}>
                ...
                {categoryChartData.length > 0 && (
                    <View className="mx-6 mb-4 bg-white rounded-[32px] p-6 border border-slate-100 shadow-sm">
                        <View className="mb-4">
                            <Text className="text-sm font-bold text-slate-800">Gastos por Categoria</Text>
                            <Text className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Despesas do Mês</Text>
                        </View>
                                ...
                                <PieChart
                                    ...
                                    centerLabelComponent={() => (
                                        <View className="items-center justify-center">
                                            <Text className="text-[10px] font-bold text-slate-400 uppercase">Despesas</Text>
                                            <Text className="text-lg font-black text-slate-800" style={isPrivacyEnabled && { color: '#cbd5e1' }}>
                                                {formatValue(summary?.currentMonth?.expense)}
                                            </Text>
                                        </View>
                                    )}
                    ...
                    <View className="mx-6 mb-4 bg-white rounded-[32px] p-6 border border-slate-100 shadow-sm">
                        ...
                        <View className="flex-row gap-2 mb-4">
                            {[
                                ...
                                <Pressable
                                    ...
                                    className={`px-3 py-1.5 rounded-full ${periodFilter === p.key ? 'bg-indigo-600' : 'bg-slate-100'}`}
                                >
                                    <Text className={`text-[10px] font-bold ${periodFilter === p.key ? 'text-white' : 'text-slate-500'}`}>
                                        {p.label}
                                    </Text>
                                </Pressable>
                            ))}
                        ...
                        <View style={{ height: 150, flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between' }}>
                            {filteredMonthlyHistory.map((item, index) => {
                                const incomeHeight = (item.income / maxBarValue) * 150;
                                const expenseHeight = (item.expenses / maxBarValue) * 150;
                ...
                {!summary && !loading && (
                    <View className="mx-6 items-center py-12">
                        ...
                        <Text className="text-sm font-bold text-slate-400 mt-4">Nenhum dado disponível</Text>
